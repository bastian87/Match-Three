...
public class Board : MonoBehaviour
{
    ...
    {
        // we use (float) because they start as integer and we can loose data if we keep them as integers.
            ...
            for (int j = 0; j < height; j++)
            {
                if (m_allGamePieces[i, j] == null && m_allTiles[i,j].tileType != TileType.Obstacle)
                {
                    ...
                    while (HasMatchOnFill(i, j))
                    {
                        ...
                        iterations++;
                        // good for any while loop problem like this possible one.
    ...
    public void ReleaseTile()
    {
        ...
        {
            SwapTiles(m_clickedTile, m_targetTile);
        }

        m_clickedTile = null;
        m_targetTile = null;
    }
    private void SwapTiles(Tile clickedTile, Tile targetTile)
    {
        StartCoroutine(SwapTilesRoutine(clickedTile, targetTile));
    }
    ...
    List<GamePiece> FindVerticalMatches(int startX, int startY, int minLength = 3)
    {
        ...
        /*
         * We replace this long version with a new one thanks to Sistem Linq framework.
         * The var let me infer the type based on wathever i set into the variable.
         * With Union i can combine two list, and avoids doubling elements.
         * The Union does return a type called Enumerable, we can convert ir to a list adding ToList() method.
         * 
